...
# reviewing the file is classified by safety.injection_guard, not by a second
# list here: this module kept its own copy and missed #2610, so ZWJ (every
# compound emoji) and ZWNJ (Persian, Arabic, Indic shaping) still counted as
# threats long after the shared class stopped treating them as such.
...
def _scan_memory_content(content: str) -> str | None:
    ...
    The invisible-character verdict comes from ``classify_injection`` so this
    scanner cannot drift from the shared ``invisible_char`` class again; the
    memory-specific ``_MEMORY_THREAT_PATTERNS`` below stay the broader,
    strict-scope list curated entries are held to.
